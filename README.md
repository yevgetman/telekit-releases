...
Current version: **0.18.0** for macOS Apple Silicon (`arm64`) and Intel
(`x86_64`).
...
- The [Claude Code](https://claude.ai/code) CLI on `PATH` (`claude`) — the
  default bridge harness. Codex, OpenCode, and SOV are supported as alternative
  harnesses, switchable in-chat with `/harness`.
...
- `VERSION`
- `SHA256SUMS`
- `telekit-runtime-0.18.0-macos-arm64.tar.gz` (Apple Silicon) **or**
  `telekit-runtime-0.18.0-macos-x86_64.tar.gz` (Intel)
